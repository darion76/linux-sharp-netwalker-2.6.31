...
int __init mx51_clocks_init(unsigned long ckil, unsigned long osc, unsigned long ckih1, unsigned long ckih2)
{
	...
	reg = __raw_readl(MXC_CCM_CSCDR1);
	reg &= ~MXC_CCM_CSCDR1_ESDHC2_MSHC2_CLK_PRED_MASK;
	reg |= 3 << MXC_CCM_CSCDR1_ESDHC2_MSHC2_CLK_PRED_OFFSET;
	__raw_writel(reg, MXC_CCM_CSCDR1);

	reg = __raw_readl(MXC_CCM_CSCDR1);
	reg &= ~MXC_CCM_CSCDR1_ESDHC2_MSHC2_CLK_PODF_MASK;
	reg |= 2 << MXC_CCM_CSCDR1_ESDHC2_MSHC2_CLK_PODF_OFFSET;
	__raw_writel(reg, MXC_CCM_CSCDR1);

	reg = __raw_readl(MXC_CCM_CSCDR3);
	reg &= ~MXC_CCM_CSCDR3_HSI2C_CLK_PRED_MASK;
	reg |= 3 << MXC_CCM_CSCDR3_HSI2C_CLK_PRED_OFFSET;
	__raw_writel(reg, MXC_CCM_CSCDR3);
	
	reg = __raw_readl(MXC_CCM_CSCDR3);
	reg &= ~MXC_CCM_CSCDR3_HSI2C_CLK_PODF_MASK;
	reg |= 2 << MXC_CCM_CSCDR3_HSI2C_CLK_PODF_OFFSET;
	__raw_writel(reg, MXC_CCM_CSCDR3);
