...
static void *mxc_clock_start (struct seq_file *m, loff_t *pos)
{
	return *pos < 1 ? (void *)1 : NULL;
}

static void *mxc_clock_next (struct seq_file *m, void *v, loff_t *pos)
{
	++*pos;
	return NULL;
}

static void mxc_clock_stop (struct seq_file *m, void *v)
{
}

static int mxc_clock_show (struct seq_file *m, void *v)
{
	struct clk *clkp;
	char        mm [32];

	list_for_each_entry(clkp, &clocks, node) {
	sprintf (mm, "%s-%d:", clkp->name, clkp->id);
	seq_printf (m, "%-20s %lu, %d", mm, clk_get_rate(clkp), clkp->usecount);
if (clkp->parent)
	seq_printf (m, ", %s-%d\n", clkp->parent->name,
	clkp->parent->id);
else
...
static const struct seq_operations mxc_clock_ops = {
	.start  = mxc_clock_start,
	.next   = mxc_clock_next,
	.stop   = mxc_clock_stop,
	.show   = mxc_clock_show
};

static int mxc_clock_read_open (struct inode *inode, struct file *file)
{
	return seq_open(file, &mxc_clock_ops);
}

static const struct file_operations mxc_clock_read_ops = {
	.open           = mxc_clock_read_open,
	.read           = seq_read,
	.llseek         = seq_lseek,
	.release        = seq_release,
};
...
#ifdef CONFIG_MACH_MX51_ERDOS
	res = proc_create ("cpu/clocks", 0, NULL, &mxc_clock_read_ops);
#else
...
#ifdef CONFIG_MACH_MX51_ERDOS
	res->proc_fops = &mxc_clock_read_ops;
#else
