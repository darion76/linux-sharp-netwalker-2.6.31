 ...
 *   4) Charge status
 *	0x03: charging
 *	0x00: not-charge, charge-complete
 *   5) Charge complete flag
 *	0x01: charge complete
 *	0x00: charging, not-charge
...
#ifdef CONFIG_MACH_MX51_ERDOS
	seq_printf(m, "%s 1.2 0x%02x 0x%02x 0x%02x 0x%02x %d%% %d.%03d V\n",
		     driver_version, APM_32_BIT_SUPPORT,
		     info.ac_line_status, info.battery_status,
		     info.battery_flag, info.battery_life,
		     (info.time / 1000), (info.time % 1000));
#else
...
#ifdef CONFIG_MACH_MX51_ERDOS /* add 090731 */
int g_apm_spi_disable = 0;
EXPORT_SYMBOL(g_apm_spi_disable);

static ssize_t proc_apm_write(struct file *file, const char __user *user,
                              size_t count, loff_t *data)
{
	...
	str[count - 1]    = '\0';
	g_apm_spi_disable = simple_strtoul(str, NULL, 0);
