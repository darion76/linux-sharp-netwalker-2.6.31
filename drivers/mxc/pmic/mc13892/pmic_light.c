 ...
 * ------------------------
 * 2009/08/21 : support suspend/resume.
...
/*
 * LED current information
 *  -1 : not use
 */
struct _pmic_light_info {
    int  level;		/* current level */
    int  high;		/* high mode */
};
static struct _pmic_light_info pmic_light_info [] = {
 { -1, -1 }, /* LIT_MAIN */
 { -1, -1 }, /* LIT_AUX */
 { -1, -1 }, /* LIT_KEY */
 { -1, -1 }, /* LIT_RED */
...
{
#ifdef CONFIG_MACH_MX51_ERDOS
	struct _pmic_light_info *info = &(pmic_light_info [0]);
	int i;
	int save = -1;
	for ( i = 0 ; i <= LIT_BLUE ; i++ , info++ ) {
		/*
		 * LED off
		 */
		if (0 < info->high) {
			save = info->high;
			mc13892_bklit_set_hi_current(i, 0);
			info->high = save;
		}
		if (0 < info->level) {
			save = info->level;
			mc13892_bklit_set_current(i, 0);
			info->level = save;
		}
	}
#endif /* CONFIG_MACH_MX51_ERDOS */
...
static int pmic_light_resume(struct platform_device *pdev)
{
	...
	for ( i = 0 ; i <= LIT_BLUE ; i++ , info++ ) {
		 ...
		 */
		if (0 < info->level) {
			mc13892_bklit_set_current(i, info->level);
		}
		if (0 < info->high) {
			mc13892_bklit_set_hi_current(i, info->high);
		}
	}
#endif /* CONFIG_MACH_MX51_ERDOS */
	return 0;
...
#ifdef CONFIG_MACH_MX51_ERDOS
	if ((LIT_MAIN <= channel) && (channel <= LIT_BLUE)) {
		pmic_light_info [channel].high = mode;
	}
#endif /* CONFIG_MACH_MX51_ERDOS */
	return PMIC_SUCCESS;
}
...
#ifdef CONFIG_MACH_MX51_ERDOS
	if ((LIT_MAIN <= channel) && (channel <= LIT_BLUE)) {
		pmic_light_info [channel].level = level;
	}
#endif /* CONFIG_MACH_MX51_ERDOS */
