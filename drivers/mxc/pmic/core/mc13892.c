 ...
 * ------------------------
 * 2009/08/12 : add pmic_event_for_suspend/_resume().
...
/*!
 * This function interrupt enable for suspend mode.
 *
 * @param 	type  enable interrupt type
 *
 * @return      OK/ERROR
 */
int pmic_event_for_suspend (type_event event)
{
	unsigned int val_bit [2];
	unsigned int event_bit = 0;
	int ret = 0;

	val_bit [0] = 0x00387fff;	/* all mask */
	val_bit [1] = 0x00c1ffff;	/* all mask */
	if (event < EVENT_1HZI) {
		event_bit = (1 << event);
		ret |= pmic_write_reg(REG_INT_MASK0, val_bit[0] & ~event_bit, val_bit[0]);
		ret |= pmic_write_reg(REG_INT_MASK1, val_bit[1], val_bit[1]);
	} else {
		event -= 24;
		event_bit = (1 << event);
		ret |= pmic_write_reg(REG_INT_MASK0, val_bit[0], val_bit[0]);
		ret |= pmic_write_reg(REG_INT_MASK1, val_bit[1] & ~event_bit, val_bit[1]);
	}
	if (ret != PMIC_SUCCESS) { 
		printk ("pmic_event_for_suspend: pmic INT_MASK0/1 error %d\n", ret);
	}
	return ret;
}

/*!
 * This function interrupt restore at resume.
 *
 * @return      OK/ERROR
 */
int pmic_event_for_resume (void)
{
	unsigned int val_bit [2];
	int ret = 0;
	...
	val_bit [0] = 0x00387fff;	/* all mask */
	val_bit [1] = 0x00c1ffff;	/* all mask */
	ret |= pmic_write_reg(REG_INT_MASK0, ~events_enabled0, val_bit [0]);
	ret |= pmic_write_reg(REG_INT_MASK1, ~events_enabled1, val_bit [1]);
	if (ret != PMIC_SUCCESS) { 
		printk ("pmic_event_for_resume: pmic INT_MASK0/1 error %d\n", ret);
	}
	return ret;
}
