...
static void apm_battery_apm_get_power_status(struct apm_power_info *info)
{
	...
	union power_supply_propval capacity, time_to_full;
#else
	 ...
	 */
	if (status.intval == POWER_SUPPLY_STATUS_CHARGING) {
		info->battery_status = 0x03;	/* charging */
	} else {
		info->battery_status = 0x00;	/* not charge */
	}
	if (status.intval == POWER_SUPPLY_STATUS_FULL) {
		info->battery_flag = 0x01;	/* charge complete */
	} else {
		info->battery_flag = 0x00;
	}
	/*
	 * Battery Voltage
	 */
	if (MPSY_PROP(VOLTAGE_NOW, &time_to_full) == 0) {
		info->time = time_to_full.intval;
	} else {
		info->time = 0;
	}
	/*
