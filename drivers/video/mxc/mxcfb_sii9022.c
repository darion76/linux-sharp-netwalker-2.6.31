...
int sii9022_hdmi_audio_init(void)
{
	unsigned char audio_info[14] = { 
		0x84, 	//frame type
		0x01, 	//version
		0x0A, 	//length
		0x0, 	//cksum
		0x01, 	//refer stream  | 2 channel
		0x00, 	//refer stream
	...
	//print(" enable sd0 ~ sd3 \n");
	i2c_smbus_write_byte_data(sii9022_client, 0x1F, 0x80);
	i2c_smbus_write_byte_data(sii9022_client, 0x1F, 0x91);
	i2c_smbus_write_byte_data(sii9022_client, 0x1F, 0xa2);
	i2c_smbus_write_byte_data(sii9022_client, 0x1F, 0xb3);

	// PCM format
	//i2c_smbus_write_byte_data(sii9022_client, 0x21, 0x00);

	//print(" write 0x24 to 48 khz \n"); 48K = 0x02, 44.1K = 0x00
	i2c_smbus_write_byte_data(sii9022_client, 0x24, 0x02);

	//print(" 24 bit \n"); 16bit=0x02 , 24b = 1011 (default)
	i2c_smbus_write_byte_data(sii9022_client, 0x25, 0x02);
	...
	// burst write audio infoframe to Reg bf-cd
	i2c_smbus_write_byte_data(sii9022_client, 0xBF, 0xC2);

	audio_info[5] = hdmi_sh_value;
	
	for( i=0; i < 14; i++ )
		cksum += audio_info[i];
	audio_info[3] = 0x100 - cksum;

	for(i=0; i<14; i++) {
		i2c_smbus_write_byte_data(sii9022_client, 0xC0+i, audio_info[i]);	
	}

#if 0
	i2c_smbus_write_byte_data(sii9022_client, 0xC0, 0x84);	//Infoframe Type Code
	i2c_smbus_write_byte_data(sii9022_client, 0xC1, 0x01);	//Version
	i2c_smbus_write_byte_data(sii9022_client, 0xC2, 0x0A);	//infoframe length
	i2c_smbus_write_byte_data(sii9022_client, 0xC3, 0x70);	//cksum
	i2c_smbus_write_byte_data(sii9022_client, 0xC4, 0x01);	//data byte1: refer stream | 2 channel
	i2c_smbus_write_byte_data(sii9022_client, 0xC5, 0x00);	//data byte2: sampling freq
	i2c_smbus_write_byte_data(sii9022_client, 0xC6, 0x00);
	i2c_smbus_write_byte_data(sii9022_client, 0xC7, 0x00);
	i2c_smbus_write_byte_data(sii9022_client, 0xC8, 0x00);
	i2c_smbus_write_byte_data(sii9022_client, 0xC9, 0x00);
	i2c_smbus_write_byte_data(sii9022_client, 0xCA, 0x00);
	i2c_smbus_write_byte_data(sii9022_client, 0xCB, 0x00);
	i2c_smbus_write_byte_data(sii9022_client, 0xCC, 0x00);
	i2c_smbus_write_byte_data(sii9022_client, 0xCD, 0x00);
#endif
	...
	i2c_smbus_write_byte_data(sii9022_client, 0xBE, 0x00);
...
int sii9022_hdmi_video_init(struct fb_var_screeninfo *var)
{
	...
	*/
	...
	if (mode == 4 ) { /* ce mode 4: 1280x720@60 Hz */
		...
		i2c_smbus_write_byte_data(sii9022_client, 0x00, 0xFA);
		i2c_smbus_write_byte_data(sii9022_client, 0x01, 0x1C); //pixel clock 74.18 - 60Hz
		i2c_smbus_write_byte_data(sii9022_client, 0x02, 0x6A);
		i2c_smbus_write_byte_data(sii9022_client, 0x03, 0x17); // v refresh rate 59.94
		i2c_smbus_write_byte_data(sii9022_client, 0x04, 0x72); 
		i2c_smbus_write_byte_data(sii9022_client, 0x05, 0x06); //total pixel 1650
		i2c_smbus_write_byte_data(sii9022_client, 0x06, 0xee); 
		i2c_smbus_write_byte_data(sii9022_client, 0x07, 0x02); //total line 750
	}
#endif
