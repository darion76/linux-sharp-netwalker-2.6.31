...
#define IOCTL_PROBE_DEV     (SIOCDEVPRIVATE + 0)
#define IOCTL_WRITE_EEPROM  (SIOCDEVPRIVATE + 1)
#define IOCTL_READ_EEPROM   (SIOCDEVPRIVATE + 2)
#define IOCTL_WRITE_EEPROM_LONG	(SIOCDEVPRIVATE + 3)
#define IOCTL_READ_EEPROM_LONG	(SIOCDEVPRIVATE + 4)

typedef struct {
    char name[16];
    unsigned short eeprom[256];
} axiod;
...
static int asix_ioctl (struct net_device *net, struct ifreq *rq, int cmd)
{
	...
	u32 size;
	axiod tiodata;
	int ret = -1,eeprom_size = 128;
	u16 i;

	switch(cmd) {
	case IOCTL_PROBE_DEV:
		strcpy(tiodata.name, "asixxisa");
		size = copy_to_user((void*)rq->ifr_ifru.ifru_data,
					(void*)&tiodata,
					sizeof(axiod));
		if(size == 0) {
			ret = 0;
		...
		break;
	case IOCTL_WRITE_EEPROM_LONG:
		eeprom_size = 256;
	case IOCTL_WRITE_EEPROM:
		size = copy_from_user(&tiodata,
				rq->ifr_ifru.ifru_data,
				sizeof(axiod));
	
		if(size > 0) {
			printk("copy from user size = %u\n", size);
			return -1;
		}
			
		if( (asix_write_cmd(dev, 0x0d, 0, 0, 0, NULL)) < 0) {
			printk("enable write eeprom failed\n");
			return -1;
		}
		...
		for(i = 0; i < eeprom_size; i++) {
			if((asix_write_cmd(dev, 0x0c, i, tiodata.eeprom[i], 0, NULL)) < 0) {
				printk("write eeprom failed %u\n", i);
				break;
			}
			msleep(5);
		}

		if( (asix_write_cmd(dev, 0x0e, 0, 0, 0, NULL)) < 0) {
			printk("disable write eeprom failed\n");
			return -1;
		}
		
		msleep(5);
		
		if( (asix_write_cmd(dev, 0x1f,0x80,0,0,NULL))<0) {
			printk("reload eeprom failed\n");
			return -1;
		}
		...
		break;
	case IOCTL_READ_EEPROM_LONG:
		eeprom_size = 256;
	case IOCTL_READ_EEPROM:
		for(i = 0; i < eeprom_size; i++) {
			if(asix_read_cmd(dev, 0x0b, i, 0, 2, &(tiodata.eeprom[i])) != 2) {
				printk("Read EEPROM failed in %d\n\r", i);
				break;
			}
		}
		if(i == eeprom_size) {
