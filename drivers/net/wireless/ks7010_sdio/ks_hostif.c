 ...
 *
 *   ks_hostif.c
 *   $Id: ks_hostif.c 822 2009-01-27 10:43:23Z sekine $
 *
...
void hostif_mib_get_confirm(ks_wlan_private *priv)
{
	...
	case DOT11_PRODUCT_VERSION:
		...
		break;
	case LOCAL_GAIN:
		memcpy(&priv->gain, priv->rxp, priv->rx_size);
...
void hostif_associate_indication(ks_wlan_private *priv)
{
	...
	buf = kmalloc(50 + 2 * (assoc_req->reqIEs_size +
				assoc_resp->respIEs_size), GFP_KERNEL);
	if (!buf)
		return;
	...
	DPRINTK(3,"IWEVENT:ASSOCINFO len=%d\n",wrqu.data.length );
